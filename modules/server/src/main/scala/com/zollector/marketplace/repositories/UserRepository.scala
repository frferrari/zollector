...
trait UserRepository {
  ...
  def update(id: UserId, user: User): Task[User]
  def delete(id: UserId): Task[Boolean]
}

class UserRepositoryLive private (quill: Quill.Postgres[SnakeCase]) extends UserRepository with QuillMappings {
  import quill.*

  inline given schema: SchemaMeta[User]  = schemaMeta[User]("users")
  inline given insMeta: InsertMeta[User] = insertMeta[User](_.createdAt, _.updatedAt)
